                    ...
                    <form action="{{ route('jadwal_pelajaran.post') }}" method="post">
                        ...
                        <div class="row">
                            <div class="col-6">
                                ...
                                <select class="form-select mb-3" name="id_guru" id="id_guru"
                                    aria-label="Default select example" required
                                    oninvalid="this.setCustomValidity('NIP - Guru tidak boleh kosong')"
                                ...
                                <select class="form-select mb-3" name="id_pelajaran" id="id_pelajaran"
                                    aria-label="Default select example" required
                            ...
                            <div class="col-6">
                                ...
                                <div class="input-group mb-3">
                                    <input type="time" class="form-control" id="jam_mengajar" name="jam_mengajar"
                                        aria-describedby="basic-addon3" required
                                        oninvalid="this.setCustomValidity('Jam mengajar tidak boleh kosong')"
                                        oninput="setCustomValidity('')">
                                </div>
                                ...
                                <div class="input-group mb-3">
                                    <input type="text" class="form-control" id="jumlah_jam" name="jumlah_jam"
                                        aria-describedby="basic-addon3" required
                                        oninvalid="this.setCustomValidity('Jumlah jam tidak boleh kosong')"
                                        oninput="setCustomValidity('')">
                                </div>

                                <label for="hari" class="form-label">Hari</label>
                                <select class="form-select mb-3" name="hari" id="hari"
                                    aria-label="Default select example" required
                                    oninvalid="this.setCustomValidity('Pelajaran tidak boleh kosong')"
                                    oninput="setCustomValidity('')">
                                    <option value="" selected>-pilih-</option>
                                    ...
                                    <form action="hapusjadwalpelajaran/{{$jp->id }}" method="POST">
                                        ...
                                        <button class="btn btn-danger" type="submit"
                                            onclick="javascript: return confirm('Anda yakin akan menghapus ini? ')"><i
                                                class="fas fa-trash-alt">Hapus</i>
                                        </button>
                                    </form>
                                    <span>
                                        <button type="button" class="btn btn-primary mx-2" data-coreui-toggle="modal"
                                            data-coreui-target="#exampleModal{{ $jp->id }}">
                                            Edit
                                        </button>
                                    </span>
                                    <div class="modal fade" id="exampleModal{{ $jp->id }}" tabindex="-1"
                                        ...
                                        <div class="modal-dialog">
                                            <div class="modal-content">
                                                ...
                                                <div class="modal-body">
                                                    <form action="{{ route('edit.jadwal_pelajaran',['id'=>$jp->id]) }}"
                                                        method="POST">
                                                        @csrf
                                                        <div class="row">
                                                            <div class="col-12">
                                                                <label for="id_guru" class="form-label">NIP -
                                                                    Guru</label>
                                                                <select class="form-select" name="id_guru" id="id_guru"
                                                                    ...
                                                                    oninput="setCustomValidity('')">
                                                                    @foreach ($guru as $g )
                                                                    <option @if($g->id == $jp->id_guru) selected @endif
                                                                        value="{{ $g->id }}">{{ $g->nama }}</option>
                                                                    @endforeach
                                                                </select>

                                                                <label for="id_kelas" class="form-label">Kelas</label>
                                                                <select class="form-select" name="id_kelas"
                                                                    id="id_kelas" aria-label="Default select example"
                                                                    required
                                                                    oninvalid="this.setCustomValidity('data tidak boleh kosong')"
                                                                    oninput="setCustomValidity('')">
                                                                    @foreach ($kelas as $k )
                                                                    <option @if($k->id == $jp->id_kelas) selected @endif
                                                                ...
                                                                <select class="form-select" name="id_pelajaran"
                                                                    id="id_pelajaran"
                                                                    aria-label="Default select example" required
                                                                    oninvalid="this.setCustomValidity('data tidak boleh kosong')"
                                                                    oninput="setCustomValidity('')">
                                                                    @foreach ($pelajaran as $p )
                                                                    <option @if($p->id == $jp->id_pelajaran) selected
                                                                        @endif value="{{ $p->id }}">{{ $p->nama }}
                                                                    </option>
                                                                    @endforeach
                                                                </select>

                                                                <label for="jam_mengajar" class="form-label">Jam
                                                                    Mengajar</label>
                                                                <div class="input-group mb-3">
                                                                    <input type="time" class="form-control"
                                                                        id="jam_mengajar" name="jam_mengajar"
                                                                        value="{{ $jp->jam_mengajar }}"
                                                                        aria-describedby="basic-addon3" required
                                                                        oninvalid="this.setCustomValidity('data tidak boleh kosong')"
                                                                        oninput="setCustomValidity('')">
                                                                </div>

                                                                <label for="jumlah_jam" class="form-label">Jumlah
                                                                    Jam</label>
                                                                <div class="input-group mb-3">
